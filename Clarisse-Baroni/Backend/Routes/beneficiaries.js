...
router.post('/', async (req, res) => {
  ...
  if (Object.keys(errors).length > 0) return res.status(422).json({ success: false, errors });
  try {
    ...
    return res.status(201).json({ success: true, message: 'Beneficiary registered successfully', data: beneficiary });
  } catch (err) {
